...
if [[ -z "${SECRET_JSON}" ]]; then
    echo "ERROR: Got empty response from Secrets Manager for '${SECRET_NAME}'." >&2
    exit 1
fi
...
# Write each key=value pair present in the JSON as a shell env var
{
...
# Lock down permissions — readable only by the current user

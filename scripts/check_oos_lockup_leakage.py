...
def _tracked_report_files() -> list[Path]:
    completed = subprocess.run(
    ...
    return [ROOT / Path(raw) for raw in completed.stdout.splitlines() if raw.strip()]
...
def main(argv: list[str] | None = None) -> int:
    ...
    mode.add_argument(
        "--tracked",
        action="store_true",
        help="Inspect tracked reports under reports/; this is the default when no files are provided.",
    )
    mode.add_argument(
        "--staged",
        action="store_true",
        help="Inspect only staged reports; intended for pre-commit hooks.",
    )
    parser.add_argument(
    ...
    parser.add_argument("files", nargs="*", help="Files to inspect; defaults to tracked reports.")
    ...
    if args.files:
        ...
        files = _staged_report_files()
    else:
        files = _tracked_report_files()
    failures = []

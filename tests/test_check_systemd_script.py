...
class CheckSystemdScriptTests(unittest.TestCase):
    ...
    def test_crypto_scan_lock_skip_exit_75_is_accepted(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp = Path(tmpdir)
            project = tmp / "project"
            scripts_dir = project / "scripts"
                    ...
                    case "$cmd" in
                      ...
                      show)
                        ...
                        case "${prop}:${unit}" in
                          ...
                          ActiveState:hawkstrade-crypto-scan.service) echo "inactive" ;;
                          SubState:hawkstrade-crypto-scan.service) echo "dead" ;;
                          Result:hawkstrade-crypto-scan.service) echo "exit-code" ;;
                          ExecMainStatus:hawkstrade-crypto-scan.service) echo "75" ;;
                          ExecMainCode:hawkstrade-crypto-scan.service) echo "1" ;;
                          InactiveEnterTimestamp:hawkstrade-crypto-scan.service) echo "Mon 2026-04-20 00:00:05 UTC" ;;
                          *) echo "" ;;
                        esac
        ...
        output = result.stdout + result.stderr
        self.assertEqual(result.returncode, 0, output)
        self.assertIn("last run skipped because trade lock was busy (exit 75)", output)
        self.assertNotIn("[FAIL] hawkstrade-crypto-scan.service", output)

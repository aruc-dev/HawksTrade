...
class TestGetConfig(unittest.TestCase):
    ...
    def test_default_strategy_enablement_profile(self):
        ...
        self.assertTrue(cfg["validation"]["cost_model"]["slippage_model_enabled"])
        self.assertEqual(cfg["validation"]["cost_model"]["slippage_multiplier"], 1.0)
        self.assertEqual(cfg["validation"]["cost_model"]["sensitivity_multipliers"], [1.0, 2.0, 3.0])
        ...
        self.assertTrue(cfg["backtest"]["minute_replay"]["enabled"])
        self.assertEqual(cfg["backtest"]["minute_replay"]["cache_dir"], "data/minute_cache")
        self.assertFalse(cfg["backtest"]["minute_replay"]["allow_synthetic_intraday"])
        self.assertTrue(cfg["backtest"]["minute_replay"]["fail_closed_on_missing"])
        ...
        self.assertTrue(cfg["slippage_model"]["enabled"])
        self.assertEqual(cfg["slippage_model"]["k_stock"], 8.0)
        self.assertEqual(cfg["slippage_model"]["k_crypto"], 15.0)
        self.assertEqual(cfg["slippage_model"]["high_cost_warning_bps"], 50.0)
        self.assertFalse(cfg["execution_policy"]["enabled"])
        self.assertEqual(cfg["execution_policy"]["policy"], "two_leg_passive_aggressive")
        self.assertEqual(cfg["execution_policy"]["per_asset_class"]["stock"]["leg1_fraction"], 0.5)
        self.assertEqual(cfg["latency_budget"]["stock_decision_to_submit_ms"], 500)
        self.assertEqual(cfg["latency_budget"]["crypto_decision_to_submit_ms"], 2000)
